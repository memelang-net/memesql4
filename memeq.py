...
TBL = {AID: DB['table_node'], AMT: DB['table_numb'], ALP: DB['table_name']}

FUNC, CLMN, TIER, TBEG, TEND = 0, 1, 2, 3, 4
Ti, T0, T1, T2, T3, T4 = -1, 0, 1, 2, 3, 4
...
OPR = { # Each operator and its meaning
	...
	I[' ']   : [XO, RID, T1, False, False],
	I[']']   : [XO, RID, T2, ' ]', False],
	I['[']   : [XO, RID, Ti, ' [', False],
	I['&']   : [XO, RID, T3, ' &', False],
	I['>>']  : [XO, RID, T3, False, False],
	I[';']   : [XO, RID, T4,  "\n", False],
	I['=']   : [YO, AID, T0, False, False],
	I['="'] : [YO, ALP, T0, False, '"'],
	I['==']  : [YO, AMT, T0, False, False],
	I['>']   : [YO, AMT, T0, False, False],
	I['<']   : [YO, AMT, T0, False, False],
	I['>=']  : [YO, AMT, T0, False, False],
	I['<=']  : [YO, AMT, T0, False, False],
...
# Input: memeQ string as "operator1operand1operator2operand2"
...
# Output: memeQ string "operator1operand1operator2operand2"
	...
	for s, expressions in enumerate(statements):
		for e, terms in enumerate(expressions):
			for t in OPRS:
				...
				memestr += K[terms[t]] if OPR[terms[t]][TBEG] is False else OPR[terms[t]][TBEG]
				if terms[t+1] is not None: memestr += str(terms[t+1])
				if OPR[terms[t]][TEND]: memestr += OPR[terms[t]][TEND]
	return memestr
...
def selectify(expressions: list[list], gids: list[int] = []) -> tuple[str, list]:
	...
	for terms in expressions:
		...
		if terms[YO]:
			...
			if col == AID:
				if terms[YV] is not None:
					...
					select 	+= f", ' ', n{n}.rid, '=', n{n}.aid"
					groupby += f", n{n}.rid, n{n}.aid"
				else:
					select 	+= f", string_agg(' ' || n{n}.rid || '=' || n{n}.aid, '')"
				

			elif col == ALP:
				select 	+= f", ' ', string_agg(n{n}.rid || '=\"' || n{n}.alp || '\"', '')"
				if terms[YV] is not None:
					where += f" AND LOWER(n{n}.alp) LIKE %s"
					params.append(terms[YV].lower())

			elif col == AMT: 
				select 	+= f", ' ', string_agg(n{n}.rid || '==' || n{n}.amt, '')"
				if terms[YV] is not None:
		...
		if tier == T4:
			...
			select	+= f", ' ]', n{n}.rid"
			join	+= f" JOIN {tbl} n{n} ON n{ln}.aid=n{n}.aid"
			where	+= f" AND n{n}.gid={gid} AND n{ln}.bid!=n{n}.bid"
			groupby += f", n{n}.rid, n{n}.bid"

		elif tier == T1:
			join	+= f" JOIN {tbl} n{n} ON n{ln}.bid=n{n}.bid"


	return f"SELECT CONCAT({select}) AS raq {join} WHERE {where} GROUP BY {groupby}", params


# Input: memeQ query string
...
# Execute and output a memeQ query
